...
var Download = function(fileId, dest) {
  dest = _.isString(dest) ? dest : null;
  return Download.getProjectById(fileId)
...
Download.getProjectById = function getProjectById(fileId) {
  var project = {};
  var deferred = Q.defer();
  authenticate().then(function(auth) {
    return Q.all([Download.getProjectTitle(fileId, auth), Download.getProjectFiles(fileId, auth)]);
  })
...
Download.getProjectTitle = function getProjectTitle(fileId, auth) {
  var deferred = Q.defer();
...
Download.getProjectFiles = function getProjectFiles(fileId, auth) {
  var deferred = Q.defer();
  var options = {
